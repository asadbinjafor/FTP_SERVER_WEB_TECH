...
function setRememberCookie($userId){
    ...
    setcookie("remember_me", $userId . ":" . $token, array(
        "expires"  => time() + 604800,
        "path"     => "/",
        "httponly" => true,
        "samesite" => "Strict"
...
function clearRememberCookie(){
    setcookie("remember_me", "", array("expires" => time() - 3600, "path" => "/", "httponly" => true, "samesite" => "Strict"));
}
...
function tryRememberLogin($mydb, $conn){
    ...
    $uid  = (int)$parts[0];
    $exp  = hash_hmac("sha256", (string)$uid, REMEMBER_SECRET);
    if(!hash_equals($exp, $parts[1])){
        return;
    }
